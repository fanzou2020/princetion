...
public class FordFulkerson {
    ...
    private boolean isFeasible(FlowNetwork G, int s, int t) {
        ...
        // check that capacity constrains are satisfied
        for (int v = 0; v < G.V(); v++) {
            for (FlowEdge e : G.adj(v)) {
                if (e.flow() < -FLOATING_POINT_EPSILON || e.flow() > e.capacity() + FLOATING_POINT_EPSILON) {
                    System.err.println("Edge does not satisfy capacity constrains: " + e);
                    return false;
                }
            }
        }
    ...
    private boolean check(FlowNetwork G, int s, int t) {
        ...
        if (!inCut(s)) {
            System.err.println("source " + s + " is not on source code side of min cut");
        }
        if (inCut(t)) {
            System.err.println("sink" + s + " is on source code side of min cut");
            return false;
        }
